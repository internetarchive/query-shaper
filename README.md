...
Attach `<query-shaper>` to a search input to turn a natural-language query
into something a search backend can actually use.
...
`@internetarchive/query-shaper`. The core behavior tracked in `SPEC.md` is
implemented: Suggestion generation (one Lucene-style string per Suggestion,
decomposed and re-rendered internally; Fields/Examples/Notes drive fielded
reformulation), the `lucene`/`url-params`/`simple-query-string` Formats,
Actions, History, the accessible popup UI, and the demo page. See
`CONTEXT.md` for the vocabulary and `docs/adr/` for the hardest-to-reverse
architectural calls.
